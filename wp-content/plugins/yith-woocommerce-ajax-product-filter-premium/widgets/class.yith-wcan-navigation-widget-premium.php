...
if ( !class_exists( 'YITH_WCAN_Navigation_Widget_Premium' ) ) {
    ...
    class YITH_WCAN_Navigation_Widget_Premium extends YITH_WCAN_Navigation_Widget {
        ...
        public function add_reset_taxonomy_link( $taxonomy, $instance ){
            ...
            if( ( yit_is_filtered_uri() || is_product_category() || is_product_taxonomy() || is_product_tag() ) && in_array( $taxonomy, $allowed_taxonomies_reset_link ) ){
                ...
                if( $show ){
                    ...
                    printf( '<span id="yith-wcan-reset-all-%1$s" class="%2$s"><a class="yith-wcan-reset-%1$s-link" href="%3$s">%4$s</a></span>',
                        $taxonomy,
        ...
        public function show_premium_widget( $args, $instance, $display_type, $terms, $taxonomy, $filter_term_field = 'slug' ) {
            ...
            if ( 'multicolor' == $display_type ) {
                ...
                foreach ( $terms as $term ) {
                    ...
                    if ( !empty( $instance['multicolor'][$term_id] ) && is_array( $instance['multicolor'][$term_id] ) && !empty( $instance['multicolor'][$term_id][0] ) ) {
                        ...
                        echo ( $count > 0 || $option_is_set ) ? '<a class="' . $a_class . '" style="' . $a_style . '" href="' . $link . '" title="' . $term->name . '" >' : '<span style="background-color:' . $instance['multicolor'][$term_id][0] . ';" >';
            ...
            elseif ( 'categories' == $display_type ) {
                ...
                $this->get_categories_list_html( $args, $tree, $taxonomy, $display_type, $query_type, $instance, $terms_type_list, $current_term, $categories_filter_operator, $is_chosen_class, $is_parent_class, $is_child_class );

                echo "</ul>";
            }
        }

        public function get_categories_list_html( $args, $terms, $taxonomy, $display_type, $query_type, $instance, $terms_type_list, $current_term, $categories_filter_operator, $is_chosen_class, $is_parent_class, $is_child_class, $level = 0 ){
            $_chosen_attributes = YITH_WCAN()->get_layered_nav_chosen_attributes();
            ...
            foreach ( $terms as $parent_id => $term_ids ) {
                ...
                if ( $query_type == 'and' ) {
                    ...
                    if ( ( ! yit_term_has_child( $term, $taxonomy ) ) && $count == 0 && ! $option_is_set ) {
                        continue;
                    }
                ...
                $link =  '';
                if( is_product_category() && ( empty( $brands_taxonomy ) || $brands_taxonomy != $taxonomy ) ){
                ...
                if( is_product_taxonomy() ){
                    if( ! in_array( $queried_object->slug, $current_filter ) ){
                        $link = add_query_arg( array( 'source_id' => $queried_object->term_id,  ), $link );
                    }

                    if( 'product_cat' == $queried_object->taxonomy ){
                        //$current_filter[]       = $queried_object->slug;
                        $_chosen_categories[]   = $queried_object->slug;
                    }
                }
                ...
                if ( isset( $_GET['source_id'] ) ) {
                    $link = add_query_arg( 'source_id', $_GET['source_id'], $link );
                }
                ...
                if( ! $skip ){
                    ...
                    if( $count > 0 || $option_is_set ) {
                        ...
                        ! $filter_is_hierarchical && printf( '<span>%s</span>', $term->name );
                    }
                ...
                if( ! empty( $term_ids ) && is_array( $term_ids ) ){
                    $temp_level = $level;
                    $temp_level++;
                    $this->get_categories_list_html( $args, $term_ids, $taxonomy, $display_type, $query_type, $instance, $terms_type_list, $current_term, $categories_filter_operator, $is_chosen_class, $is_parent_class, $is_child_class, $temp_level );
                }
            }
        }
