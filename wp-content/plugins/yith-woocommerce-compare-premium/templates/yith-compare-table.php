...
<div id="yith-woocompare" class="woocommerce">
	...
	<?php
        if ( empty( $products ) ) {
            echo '<p>' . apply_filters( 'yith_woocompare_empry_compare_message', __( 'Нет продуктов добавленых в таблицу сравнения.', 'yith-woocommerce-compare' ) ) . '</p>';
            return;
        }
    ?>
                        ...
                        <td class="<?php echo $product_class; ?>"><?php
                            switch( $field ) {
                                ...
                                case 'product_info':
                                    ...
                                    if( ! $share )
                                        echo '<div class="remove"><a href="'. $yith_woocompare->obj->remove_product_url( $product->id ) . '" data-is_page="1" data-product_id="'. $product->id . '">' .  __( 'удалить', 'yith-woocommerce-compare' ) . '</a></div>';
