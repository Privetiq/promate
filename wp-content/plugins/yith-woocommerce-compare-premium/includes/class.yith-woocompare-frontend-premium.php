    ...
    class YITH_Woocompare_Frontend_Premium extends YITH_Woocompare_Frontend {
	    ...
	    public function enqueue_premium_scripts(){
		    ...
		    wp_localize_script( 'yith-woocompare-main', 'yith_woocompare_premium', array(
			    'is_page' => get_option( 'yith_woocompare_use_page_popup' ) == 'page' ? true : false,
			    'page_url' => $this->get_compare_page_url(),
			    'im_in_page' => is_page( get_option( 'yith-woocompare-page-id' ) ),
			    'view_label'    => __( 'View Page', 'yith-woocommerce-compare' ),
			    'actionfilter' => $this->action_filter,
			    'num_related'   => get_option( 'yith-woocompare-related-visible-num' ),
			    'autoplay_related'  => get_option( 'yith-woocompare-related-autoplay' ) == 'yes' ? true : false,
	    ...
	    public function add_logo_to_compare( $products_list = array() ){
		    ...
		    $in_page  = get_option( 'yith-woocompare-table-image-in-page' ) == 'yes' ? true : false;
		    $in_popup = get_option( 'yith-woocompare-table-image-in-popup' ) == 'yes' ? true : false;

		    if( ! $image || ( is_page( get_option( 'yith-woocompare-page-id' ) ) && ! $in_page ) || ( isset( $_REQUEST['iframe'] ) && ! $in_popup ) ) {
			    return;
		    }
	    ...
	    public function exclude_product_from_list( $products ) {
		    ...
		    $inverse      = get_option( 'yith_woocompare_excluded_category_inverse' ) == 'yes' ? true : false;
	    ...
	    public function filter_table_url( $link, $product_id ) {
		    ...
		    if( get_option( 'yith_woocompare_use_page_popup' ) != 'page' ) {
			    return $link;
		    }
	    ...
	    public function social_share() {
		    ...
		    $enable = get_option( 'yith-woocompare-enable-share' ) == 'yes' ? true : false;
		    ...
		    $in_page  = get_option( 'yith-woocompare-share-in-page' ) == 'yes' ? true : false;
		    $in_popup = get_option( 'yith-woocompare-share-in-popup' ) == 'yes' ? true : false;

		    if( ( is_page( get_option( 'yith-woocompare-page-id' ) ) && ! $in_page ) || ( isset( $_REQUEST['iframe'] ) && ! $in_popup ) ) {
			    return;
		    }
		    ...
		    $args = array(
		        ...
		        'share_link_url' => esc_url_raw( add_query_arg( 'yith_compare_prod', $products_base64, get_permalink( get_option( 'yith-woocompare-page-id' ) ) ) ),
        ...
        public function related_product() {
	        ...
	        // check for page/popup
	        $in_page  = get_option( 'yith-woocompare-related-in-page' ) == 'yes' ? true : false;
	        $in_popup = get_option( 'yith-woocompare-related-in-popup' ) == 'yes' ? true : false;
